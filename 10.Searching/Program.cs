    ...
    internal class Searching
    {
        ...
        public static int BinarySearch<T>(in IList<T> list, in T item) where T : IComparable<T>
        {
            ...
            while (low <= high)
            {
                ...
                int compare = list[middle].CompareTo(item); //미들의 값과 찾으려는 항목 비교 

                if (compare < 0)                                // 찾는 항목이 더 큰경우
                {
                    low = middle + 1;                       // 범위 미들위로 올리기
                }
                else if (compare > 0)                           //찾는 항목이 더 작은경우
                {
                    high = middle - 1;                      //범위 미들 아래로 내리기
                }
                else
                {
                    return middle;
