namespace _12.PathFinding
{
    internal class AStar
    {
        ...
        public static bool PathFinding(in bool[,] tileMap, in Point start, in Point end, out List<Point> path)
        {
            ...
            // 0. 시작 정점을 생성하여 추가
            ASNode startNode = new ASNode(start, new Point(), 0, Heuristic(start, end));        // A스타노드의 구성 :  (정점,부모노드,현재이동한거리,예상이동거리)
        ...
        {
            int xSize = Math.Abs(start.x - end.x);  // 가로로 가야하는 횟수              start.x>end.x ? start.x - end.x : end.x - start.x;
